...
class RepoData:
    ...
    def process_archive_url(self):
        ...
        if "$" in self.archive_url:
            print(
                "Warning: found unprocessed variable in archive url " + self.archive_url
            )
...
class DataDownloader:
    ...
    def _get_dist_repos(self, distro_name, dist_obj):
        ...
        for version in dist_obj.get("versions", [{}]):
            for repo_name, repo_obj in dist_obj.get("repos").items():
                ...
                url_md["areas"] = repo_obj.get(
                    "areas"
                )  # Only apt-based dists have this
    ...
    async def _get_files(self, repos):
        ...
        files.extend(itertools.chain(*await asyncio.gather(*requests_futures)))
        return files
                ...
                case "apt":
                    ...
                    for area in repo.md["areas"]:
                        yield FileData(
                            repo,
                            os.path.join(
                                repo.archive_url, area, "binary-amd64", "Packages.gz"
                            ),
    ...
    async def query_download_size(self):
        ...
        for result in results:
            if result.ok:
                ...
                print(
                    f"Warning: request {result.url} returned HTTP code {result.status}"
                )
    ...
    async def download_files(self):
        ...
        if self.total_download_size:
            ...
            print(
                "query_download_size() not called. Total download size will not be accurate until all requests have been made (5 concurrently)"
            )
            ...
            for filedata in self.files:
                if os.path.exists(filedata.file):
                    if (
                        not self.force
                        and ask(
                            f"Something exists at location {filedata.file}. Do you want to overwrite it ?",
                            "n",
                        )
                        == "y"
                    ):
                ...
                tasks.append(
                    self._download_and_save(
                        bar=bar, session=session, url=filedata.url, file=filedata.file
                    )
                )
    ...
    def filter(self, distro_name=None, distro_version=None):
        ...
        def __filter_file(filedata):
            if distro_name and distro_name != filedata.repodata.md.get("distro"):
                return False
            if distro_version and distro_version != filedata.repodata.md.get(
                "version_id"
            ):
                return False
            return True
